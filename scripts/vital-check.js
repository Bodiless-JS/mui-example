...
const vitalNextPath = 'packages/muiExample';
...
if (vitalHash !== vitalNextHash) {
  ...
  // Log file missing from muiExample
  vitalMoreFiles.forEach(file => differences.push(
    `${RED}\t--vital-- contains the file ${file}, it is missing from muiExample.${NC}`
  ));

  // Log file missing from --vital--
  vitalNextMoreFiles.forEach(file => differences.push(
    `${RED}\tmuiExample contains the file ${file}, it is missing from --vital--${NC}`
  ));
  ...
  console.error(`${RED}The packages __vital__ and muiExample differ in these files:${NC}\n`);
  differences.forEach(difference => console.error(difference));
  process.exit(-1);
}

console.error(`${GREEN}The packages __vital__ and muiExample are identical.${NC}`);
